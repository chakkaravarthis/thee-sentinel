...
def handle_devops_issue(incident: dict, config: dict) -> dict:
    ...
    return {
        "action_taken": f"ADO ticket created. Engineer notified with exact file path and variable name.",
...
def _create_ado_ticket(root_cause: str, affected_file: str, fix: str, config: dict) -> str:
    ...
    body = [
        {"op": "add", "path": "/fields/System.Title",
         "value": f"[Thee Sentinel] DevOps Config Issue: {root_cause[:80]}"},
        {"op": "add", "path": "/fields/System.Description",
         "value": f"<b>Root Cause:</b> {root_cause}<br><b>File to Update:</b> {affected_file}<br><b>Fix:</b> {fix}"},
        {"op": "add", "path": "/fields/System.Tags", "value": "thee-sentinel;auto-created"}
    ]
    resp = requests.post(url, headers=headers, json=body)
    return resp.json().get("_links", {}).get("html", {}).get("href", "https://dev.azure.com/thee-sentinel")
